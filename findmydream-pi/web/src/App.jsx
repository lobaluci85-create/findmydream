...
import { initPi, loginWithPi, getJournal, saveDream, getCollective, payForDeepReading } from "./pi.js";
...
async function loadJournal(){
  try { const raw = localStorage.getItem(JKEY); return raw ? JSON.parse(raw) : []; }
  catch { return []; }
}
async function persistJournal(arr){
  try { localStorage.setItem(JKEY, JSON.stringify(arr.slice(0,500))); }
...
export default function FindMyDream(){
  ...
  async function signIn(){
    ...
    catch (e) { alert("To sign in, open FindMyDream inside the Pi Browser."); }
  }

  // Journal: from the backend when signed in with Pi, otherwise from this device.
  useEffect(() => {
    let alive = true;
    (async () => {
      if (user) {
        try { const rows = await getJournal(); if (alive) setJournal(rows); } catch { if (alive) setJournal([]); }
        return;
      }
  ...
  // Live collective numbers when a backend is reachable; otherwise the seeded placeholder.
  useEffect(() => {
    getCollective().then(c => { if (c && c.symbols && c.symbols.length) setCollective(c.symbols); }).catch(()=>{});
  }, [user]);
  ...
  function interpret(){
    ...
    setTimeout(() => {
      ...
      if (user) {
        saveDream(entry).then(({ id }) => setJournal(j => [{ ...entry, id }, ...j])).catch(() => setJournal(j => [entry, ...j]));
      } else {
      ...
      <div style={{ position:"relative" }}>
        ...
        <div className="max-w-2xl mx-auto px-5 pb-20" style={{ position:"relative" }}>
          <header className="flex items-center justify-between pt-7 pb-6">
            ...
            <div className="flex items-center gap-3">
              {user ? (
                <span style={{ fontFamily:"Space Mono, monospace", fontSize:11, color:C.gold }}>@{user.username}</span>
              ) : (
                <button onClick={signIn} className="text-xs px-3 py-1.5 rounded-full" style={{ background:"transparent", color:C.gold, border:`1px solid ${C.line}`, fontWeight:600 }}>Sign in with Pi</button>
              )}
